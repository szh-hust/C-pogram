...
int  outputLetterNum(char ch,int x,int y,int size,int color)
{
		...
		case 16 :
				  {
					...
					hzk_p = fopen("c:\\test\\hzk\\hzk16", "rb");
					 if(hzk_p==NULL)
					 {
					...
					while(*s!=NULL)
					{
						...
						while (x<640-flag && (*s!=NULL))
						{
							...
							if (outputLetterNum(*s, x, y0,flag,color) == 1)
							{
								x = x + part;
								s += 1;
		...
		case 24 :
				  {
					...
					//hzk_p = fopen("c:\\myCCD\\SOURSE\\hz\\hzk24s","rb");
					 hzk_p = fopen("c:\\test\\hzk\\hzk24s", "rb");
					if (hzk_p==NULL)
					{
						settextjustify(LEFT_TEXT,TOP_TEXT);          //左部对齐，顶部对齐
		...
		case 32 :
				  {
					...
					hzk_p = fopen("c:\\test\\hzk\\hzk32s", "rb");
					 if(hzk_p==NULL)
					 {
						settextjustify(LEFT_TEXT,TOP_TEXT);          //左部对齐，顶部对齐
						settextstyle(GOTHIC_FONT,HORIZ_DIR,3);					//黑体笔划输出，水平输出，24*24点阵						
		...
		case 48:
				  {
					...
					hzk_p = fopen("c:\\test\\hzk\\hzk48s", "rb");
					 if(hzk_p==NULL)
					 {
						settextjustify(LEFT_TEXT,TOP_TEXT);          //左部对齐，顶部对齐
						settextstyle(GOTHIC_FONT,HORIZ_DIR,3);					//黑体笔划输出，水平输出，24*24点阵
